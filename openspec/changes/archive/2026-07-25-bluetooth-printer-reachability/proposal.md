...
Related bug: when Bluetooth print fails on the split-pay settle path, **the payment is already committed** but the thrown receipt error is caught by `onPay` before `emit('settled')`, so the UI **stays on the split-pay screen** (table/collective/register) even though the order/account is settled. Full-order pay (`PayOrderView`) has the same coupling: a receipt throw can block navigation to the hub.
...
- **Decouple settle completion from receipt outcome**: after a successful payment, always continue (emit `settled` / navigate / reload) even if receipt printing fails or is cancelled; receipt errors must not roll back or strand the settle UI.
...
- `event-bluetooth-printing`: Prefer Bluetooth only when the selected printer is reachable; otherwise fall back to network/station printer selection for payment receipts, vouchers, and shift receipts. Settlement UI completion must not depend on receipt print success.

## Impact

- **Android app**: `BluetoothPrinterBridge.kt` (new probe method; shared connect helper with timeout); possibly `android/README.md`.
- **Pi frontend**: `androidPrinter.ts`, `paymentReceiptPrompt.ts`, `voucherPrintDestination.ts`, `useShiftSession.ts`, `SplitPaySettleScreen.vue`, `PayOrderView.vue`, and related Vitest coverage.
