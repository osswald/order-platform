# pi-operational-restore Specification

## Purpose
Operational restore from the cloud open-state mirror must not resurrect finished kitchen work or undo local settlement on a live Pi, while still supporting empty-Pi / appliance takeover when there is no conflicting local completion.
