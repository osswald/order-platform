# cash-shift-session-history Specification

## Purpose
Cash shift sessions for waiters and registers are identified by a durable UUID so an event can retain a full history of sequential shifts while still allowing at most one open shift per subject.
