...
function mountWithCapturedTableProps(props: Record<string, unknown>) {
  const captured = ref<CapturedTableProps | null>(null)
  const DataTableStub = defineComponent({
    inheritAttrs: false,
    props: ['items', 'itemsPerPage', 'mobileBreakpoint'],
    setup(tableProps) {
      captured.value = tableProps as CapturedTableProps
      return () => null
    },
  })
