...
describe('eventPaymentTypes', () => {
  it('orders types according to PAYMENT_TYPE_ORDER', () => {
    expect(eventPaymentTypes({ payment_types: ['twint', 'cash', 'sumup'] })).toEqual([
      'cash',
      'twint',
      'sumup',
    ])
...
describe('eventTwintQrDataUrl', () => {
  it('accepts data URLs only', () => {
    expect(eventTwintQrDataUrl({ twint_qr_data_url: 'data:image/png;base64,abc' })).toBe(
      'data:image/png;base64,abc',
    )
    expect(eventTwintQrDataUrl({ twint_qr_data_url: 'https://example.com/x' })).toBeNull()
  })
})
