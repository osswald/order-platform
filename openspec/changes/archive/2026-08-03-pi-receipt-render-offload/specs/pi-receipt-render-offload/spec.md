...
Creating a queued network `PrintJob` for station, customer-pickup, voucher (network), payment-receipt, or cash-drawer jobs MUST NOT require building the final ESC/POS payload on the HTTP request path. The system SHALL persist enough render context with the job for the print worker to build the payload later, then send it. Until render succeeds, the job MUST remain eligible for worker processing (not marked `sent`).
