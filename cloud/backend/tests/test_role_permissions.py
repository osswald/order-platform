...
def _setup_tenant_with_org():
    ...
    try:
        ...
        db.add(hc)
        db.flush()
        org = Organisation(name="RBAC Org", country="CH", hire_company_id=hc.id, currency="CHF")
        db.add(org)
        db.flush()
        tenant_admin = User(
...
def test_tenant_admin_can_create_and_delete_organisation():
    hc_id, _ = _setup_tenant_with_org()
    headers = {"Authorization": f"Bearer {_token('tenant-admin@rbac.test')}"}
    created = client.post(
        "/organisations/",
        headers=headers,
        json={"name": "New Org", "country": "CH", "currency": "CHF"},
    )
...
def test_organisation_admin_cannot_create_organisation():
    ...
    created = client.post(
        "/organisations/",
        headers=headers,
        json={"name": "Blocked Org", "country": "CH", "currency": "CHF"},
    )
    assert created.status_code == 403


def test_organisation_admin_can_update_assigned_organisation():
    _, org_id = _setup_tenant_with_org()
    headers = {"Authorization": f"Bearer {_token('org-admin@rbac.test')}"}
    updated = client.put(
