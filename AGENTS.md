...
- `pi/.env` — created from `pi/.env.example`; set `SYNC_ENABLED=0` and `PRINT_TO_FILE=1` for local dev without cloud pairing

### Running tests

- **Pi backend** (all pass): `cd pi/backend && python3 -m pytest tests/ -v`
