...
function mountTaxCodes(countryIdRef) {
  let result
...
describe('formatTaxCodeLabel', () => {
  it('includes current rate percent in label', () => {
    const label = formatTaxCodeLabel({
      name: 'Normalsatz',
      rates: [{ rate_percent: 8.1, valid_from: '2020-01-01', valid_to: null }],
    })
    expect(label).toBe('Normalsatz (8,1%)')
  })

  it('returns name only when no rates', () => {
    expect(formatTaxCodeLabel({ name: 'Zero', rates: [] })).toBe('Zero')
  })
  ...
  it('ignores future rates', () => {
    ...
    const label = formatTaxCodeLabel({
      name: 'Alt',
      rates: [
        { rate_percent: 7.7, valid_from: '2018-01-01', valid_to: null },
        { rate_percent: 99, valid_from: future.toISOString().slice(0, 10), valid_to: null },
      ],
    })
