...
After connecting with an API key, **SumUp devices** shows merchant name, merchant code, country, and whether SumUp reports the account as **Sandbox** or **Live** (`GET /v1/merchants/{merchant_code}` `sandbox` flag). One API key often spans several merchants (live + sandboxes); connect lists SumUp memberships and asks you to pick one when there is more than one — do not rely on SumUp `/me`, which returns only the default live merchant.

Connect and **Update API key** (same merchant) import that merchant’s Cloud API reader catalog (`GET …/v0.1/merchants/{merchant_code}/readers`). Opening **SumUp devices** re-syncs the same catalog: new remote readers are added (label from SumUp `name`, else serial, else “Solo”), existing Vendiqo labels are kept, and local rows whose `reader_id` is no longer on SumUp are removed (cash-register defaults for those ids are cleared). If SumUp is unreachable or the list payload is malformed, the last local snapshot is kept. Hover a reader label for live telemetry (`GET …/readers/{id}/status`: online, battery, connection, firmware); serial/model still show when that call fails. Use **Disconnect** to clear credentials and local reader rows. Pair additional Solos from the page with a pairing code when the device is not yet in the SumUp catalog.
