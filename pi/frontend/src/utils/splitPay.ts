export interface SplitPayGroup {
  basketQty: number
  totalQty: number
...
export function groupBasketCents(g: SplitPayGroup): number {
...
export function sumGroupBasketCents(groups: SplitPayGroup[] | null | undefined): number {
  return (groups || []).reduce((s, g) => s + groupBasketCents(g), 0)
}

export function sumVoucherCreditCents(redemptions: VoucherRedemption[] | null | undefined): number {
...
export function basketCentsAfterVoucher(rawBasketCents: number, voucherCreditCents: number): number {
